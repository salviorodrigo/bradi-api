...
describe('CodigoUnidadeFederativa', function () {
    test('Should succeed if is declared', function () {
        $nameSpace = 'BradiApi\\Domain\\Invoices\\NFe\\v4_00\\ValueObjects';
        $sut = $nameSpace . '\\CodigoUnidadeFederativa';
        expect(class_exists($sut))->toBeTrue();
    });

    test('Should succeed if extends DFeElement', function () {
        $sut = new CodigoUnidadeFederativa('parentTag');
    ...
    describe('properties', function () {
        describe('FIELD_NAME', function () {
            test('Should be set correctly', function () {
                $reflection = new ReflectionClass(CodigoUnidadeFederativa::class);
    ...
    describe('methods', function () {
        describe('validateTagAttributes', function () {
            test('Should fail if some attribute is provided', function () {
                $targetClass = new CodigoUnidadeFederativa('parentTag');
        ...
        describe('validateTagValue', function () {
            test('Should succeed with valid codes', function (string $candidate) {
                ...
                $CodigoUnidadeFederativa = new CodigoUnidadeFederativa('parentTag');
                $sut = new ReflectionMethod($CodigoUnidadeFederativa, 'validateTagValue');
                $sutResponse = $sut->invoke($CodigoUnidadeFederativa, $element);
