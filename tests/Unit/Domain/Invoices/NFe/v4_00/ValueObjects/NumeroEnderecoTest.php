...
describe('NumeroEndereco', function () {
    ...
    describe('::parse()', function () {
        test('Should succeed with dataset :dataset', function ($candidate) {
            $xmlString = $candidate === '' ? '' : '<' . NumeroEndereco::TAG_NAME . ">{$candidate}</" . NumeroEndereco::TAG_NAME . '>';
        ...
        })->with(datasets('dfes.nfe.value_tags.' . NumeroEndereco::TAG_NAME . '.valid'));

        test('Should fail with data set :dataset', function ($candidate) {
            $xmlString = '<' . NumeroEndereco::TAG_NAME . ">{$candidate}</" . NumeroEndereco::TAG_NAME . '>';
            $sutResponse = $this->sut->parse($xmlString);
        ...
        })->with(datasets('dfes.nfe.value_tags.' . NumeroEndereco::TAG_NAME . '.invalid'));

        test('Should fail if attributes is provided', function ($candidate) {
            $xmlString = '<' . NumeroEndereco::TAG_NAME . " fake=\"attribute\">{$candidate}</" . NumeroEndereco::TAG_NAME . '>';
            $sutResponse = $this->sut->parse($xmlString);
            expect($sutResponse)->toBeInstanceOf(Result::class);
            if ($sutResponse->isSuccess()) {
                $this->fail(json_encode($sutResponse->getData()));
        ...
        test('Should fail if elements is provided', function ($candidate) {
            $xmlString = '<' . NumeroEndereco::TAG_NAME . ">{$candidate}<fake>element</fake></" . NumeroEndereco::TAG_NAME . '>';
            $sutResponse = $this->sut->parse($xmlString);
            expect($sutResponse)->toBeInstanceOf(Result::class);
            if ($sutResponse->isSuccess()) {
                $this->fail(json_encode($sutResponse->getData()));
