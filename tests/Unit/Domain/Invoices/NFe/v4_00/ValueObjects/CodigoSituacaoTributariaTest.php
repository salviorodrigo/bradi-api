...
describe('CodigoSituacaoTributaria', function () {
    ...
    describe('::parse()', function () {
        test('Should succeed with dataset :dataset', function ($candidate) {
            $xmlString = $candidate === '' ? '' : '<' . CodigoSituacaoTributaria::TAG_NAME . ">{$candidate}</" . CodigoSituacaoTributaria::TAG_NAME . '>';
        ...
        })->with(datasets('dfes.nfe.value_tags.' . CodigoSituacaoTributaria::TAG_NAME . '.valid'));

        test('Should fail with data set :dataset', function ($candidate) {
            $xmlString = '<' . CodigoSituacaoTributaria::TAG_NAME . ">{$candidate}</" . CodigoSituacaoTributaria::TAG_NAME . '>';
            $sutResponse = $this->sut->parseFromXmlElement(xml_to_element($xmlString));
        ...
        })->with(datasets('dfes.nfe.value_tags.' . CodigoSituacaoTributaria::TAG_NAME . '.invalid'));

        test('Should fail if attributes is provided', function ($candidate) {
            $xmlString = '<' . CodigoSituacaoTributaria::TAG_NAME . " fake=\"attribute\">{$candidate}</" . CodigoSituacaoTributaria::TAG_NAME . '>';
            $sutResponse = $this->sut->parseFromXmlElement(xml_to_element($xmlString));
            expect($sutResponse)->toBeInstanceOf(Result::class);
            if ($sutResponse->isSuccess()) {
                $this->fail(json_encode($sutResponse->getData()));
        ...
        test('Should fail if elements is provided', function ($candidate) {
            $xmlString = '<' . CodigoSituacaoTributaria::TAG_NAME . ">{$candidate}<fake>element</fake></" . CodigoSituacaoTributaria::TAG_NAME . '>';
            $sutResponse = $this->sut->parseFromXmlElement(xml_to_element($xmlString));
            expect($sutResponse)->toBeInstanceOf(Result::class);
            if ($sutResponse->isSuccess()) {
                $this->fail(json_encode($sutResponse->getData()));
