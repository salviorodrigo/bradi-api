...
describe('NomeMunicipio', function () {
    test('Should succeed if is declared', function () {
        ...
        $sut = $nameSpace . '\\NomeMunicipio';
        expect(class_exists($sut))->toBeTrue();
    });

    test('Should succeed if extends DFeelement', function () {
        $sut = new NomeMunicipio('parentTag');
    ...
    describe('properties', function () {
        describe('FIELD_NAME', function () {
            test('Should be set correctly', function () {
                $reflection = new ReflectionClass(NomeMunicipio::class);
                $reflectedProperty = $reflection->getConstant('FIELD_NAME');
                expect($reflectedProperty)->toBe('xMun');
            });
        });
    });
    ...
    describe('methods', function () {
        describe('validateTagAttributes', function () {
            test('Should fail if some attribute is provided', function () {
                $targetClass = new NomeMunicipio('parentTag');
        ...
        describe('validateTagValue', function () {
            test('Should succeed in border cases', function (string $candidate) {
                ...
                $nomeMunicipio = new NomeMunicipio('parentTag');
                $sut = new ReflectionMethod($nomeMunicipio, 'validateTagValue');
                $sutResponse = $sut->invoke($nomeMunicipio, $element);
            ...
            test('Should fail if a text value with invalid spaces is provided', function (string $candidate) {
                ...
                'leading space' => ' CITY',
                'trailing space' => 'CITY ',
                'nested spaces' => 'CITY WITH  SPACES',
            ]);
        });
    });
});
