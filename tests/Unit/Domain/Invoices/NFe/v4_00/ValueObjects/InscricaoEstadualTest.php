...
describe('InscricaoEstadual', function () {
    ...
    describe('::parse()', function () {
        test('Should succeed with dataset :dataset', function ($candidate) {
            $xmlString = $candidate === '' ? '' : '<' . InscricaoEstadual::TAG_NAME . ">{$candidate}</" . InscricaoEstadual::TAG_NAME . '>';
        ...
        })->with(datasets('dfes.nfe.value_tags.' . InscricaoEstadual::TAG_NAME . '.valid'));

        test('Should fail with data set :dataset', function ($candidate) {
            $xmlString = '<' . InscricaoEstadual::TAG_NAME . ">{$candidate}</" . InscricaoEstadual::TAG_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
        ...
        })->with(datasets('dfes.nfe.value_tags.' . InscricaoEstadual::TAG_NAME . '.invalid'));

        test('Should fail if attributes is provided', function ($candidate) {
            $xmlString = '<' . InscricaoEstadual::TAG_NAME . " fake=\"attribute\">{$candidate}</" . InscricaoEstadual::TAG_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
        ...
        test('Should fail if elements is provided', function ($candidate) {
            $xmlString = '<' . InscricaoEstadual::TAG_NAME . ">{$candidate}<fake>element</fake></" . InscricaoEstadual::TAG_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
