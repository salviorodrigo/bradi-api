...
describe('SiglaUF', function () {
    ...
    beforeEach(function () {
        /** @var TestCase $this */
        $this->sut = new SiglaUF('');
    });

    describe('::parse()', function () {
        test('Should succeed with dataset :dataset', function ($candidate) {
            $xmlString = $candidate === '' ? '' : '<' . SiglaUF::FIELD_NAME . ">{$candidate}</" . SiglaUF::FIELD_NAME . '>';
            ...
            expect($sutResponse->getData())->toBeInstanceOf(SiglaUF::class);
            expect($sutResponse->getData()->value)->toBe($candidate);
            expect((string) $sutResponse->getData())->toBe($xmlString);
        })->with(datasets('dfes.nfe.value_tags.' . SiglaUF::FIELD_NAME . '.valid'));

        test('Should fail with data set :dataset', function ($candidate) {
            $xmlString = '<' . SiglaUF::FIELD_NAME . ">{$candidate}</" . SiglaUF::FIELD_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
        ...
        })->with(datasets('dfes.nfe.value_tags.' . SiglaUF::FIELD_NAME . '.invalid'));

        test('Should fail if attributes is provided', function ($candidate) {
            $xmlString = '<' . SiglaUF::FIELD_NAME . " fake=\"attribute\">{$candidate}</" . SiglaUF::FIELD_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
        ...
        test('Should fail if elements is provided', function ($candidate) {
            $xmlString = '<' . SiglaUF::FIELD_NAME . ">{$candidate}<fake>element</fake></" . SiglaUF::FIELD_NAME . '>';
            $xmlElement = new Element(new FakeValidationService);
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
