...
describe('FinalidadeNF', function () {
    ...
    describe('::parse()', function () {
        test('Should succeed with dataset :dataset', function ($candidate) {
            $xmlString = $candidate === '' ? '' : '<' . FinalidadeNF::FIELD_NAME . ">{$candidate}</" . FinalidadeNF::FIELD_NAME . '>';
        ...
        })->with(datasets('dfes.nfe.value_tags.' . FinalidadeNF::FIELD_NAME . '.valid'));

        test('Should fail with data set :dataset', function ($candidate) {
            $xmlString = '<' . FinalidadeNF::FIELD_NAME . ">{$candidate}</" . FinalidadeNF::FIELD_NAME . '>';
            $xmlElement = new Element;
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
        ...
        })->with(datasets('dfes.nfe.value_tags.' . FinalidadeNF::FIELD_NAME . '.invalid'));

        test('Should fail if attributes is provided', function ($candidate) {
            $xmlString = '<' . FinalidadeNF::FIELD_NAME . " fake=\"attribute\">{$candidate}</" . FinalidadeNF::FIELD_NAME . '>';
            $xmlElement = new Element;
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
        ...
        test('Should fail if elements is provided', function ($candidate) {
            $xmlString = '<' . FinalidadeNF::FIELD_NAME . ">{$candidate}<fake>element</fake></" . FinalidadeNF::FIELD_NAME . '>';
            $xmlElement = new Element;
            $xmlElement->parse($xmlString);
            $sutResponse = $this->sut->parseFromXmlElement($xmlElement);
            expect($sutResponse)->toBeInstanceOf(Result::class);
