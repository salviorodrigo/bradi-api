...
describe('IdentificadorOperacaoConsumidorFinal', function () {
    test('Should succeed if is declared', function () {
        $nameSpace = 'BradiApi\\Domain\\Invoices\\NFe\\v4_00\\ValueObjects';
        $sut = $nameSpace . '\\IdentificadorOperacaoConsumidorFinal';
        expect(class_exists($sut))->toBeTrue();
    });

    test('Should succeed if extends DFeElement', function () {
        $sut = new IdentificadorOperacaoConsumidorFinal('parentTag');
    ...
    describe('properties', function () {
        describe('FIELD_NAME', function () {
            test('Should be set correctly', function () {
                expect(IdentificadorOperacaoConsumidorFinal::FIELD_NAME)->toBe('indFinal');
            });
        });
    });
    ...
    describe('methods', function () {
        describe('validateTagAttributes', function () {
            test('Should fail if some attribute is provided', function () {
                $targetClass = new IdentificadorOperacaoConsumidorFinal('parentTag');
        ...
        describe('validateTagValue', function () {
            test('Should succeed with valid numeric values', function (string $candidate) {
                $element = new Element;
                $element->name = IdentificadorOperacaoConsumidorFinal::FIELD_NAME;
                $element->value = $candidate;
                $instance = new IdentificadorOperacaoConsumidorFinal('parentTag');
